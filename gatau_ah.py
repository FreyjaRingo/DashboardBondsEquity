...
def calculate_metrics(price_data, benchmark_series, risk_free_rate):
    ...
    volatility_1sd = returns.std() * np.sqrt(252)
    volatility_2sd = volatility_1sd * 2 
    ...
    metrics_df = pd.DataFrame({
        ...
        'Return_1M': return_1m,
        'Return_3M': return_3m,
        'Volatility': volatility_2sd, 
        'Sharpe_Ratio': sharpe_ratio,
        'Beta': beta,
        'Alpha': alpha
    })
    
    # --- 12 Metrik Konsistensi (Count + Streak) ---
    consist_results = {}
    ...
    # --- 4 Metrik Climber (Perpindahan Peringkat) ---
    daily_ranks = returns.rank(axis=1, ascending=False, method='min')
...
def calculate_rolling_timeseries(price_data, benchmark_series, risk_free_rate, window=63):
    ...
    volatility_ts = fund_returns.rolling(window=window).std() * np.sqrt(252) * 2
...
def calculate_ranking_scores(metrics_df, weights=None):
    """Menghitung skor komposit dari 24 metrik (termasuk 4 metrik momentum perpindahan peringkat)."""
    # Bobot disebar rata ke 24 metrik (1/24 atau ~4.16% per metrik)
    w = 1.0 / 24.0
    if weights is None:
        weights = {
            'Total_Return': w, 'Return_1W': w, 'Return_1M': w, 'Return_3M': w,
...
def get_7d_ranking_history(price_data, benchmark_series, risk_free_rate, custom_weights=None):
...
with st.sidebar:
    ...
    if st.session_state.data_loaded:
        ...
        date_option = st.selectbox(
            ...
            options=["1 Bulan", "3 Bulan", "6 Bulan", "1 Tahun", "Gunakan Semua Data (Sesuai Kalender)"],
            index=3, 
        ...
        scoring_mode = st.selectbox(
            ...
            options=["Balanced (Semua Metrik)", "Fokus Return (Profit)", "Fokus Risiko & Rasio", "Fokus Konsistensi", "Fokus Momentum (Climbers)"],
            index=0,
...
if date_option == "1 Bulan":
    cutoff_days = 22 # 30 hari kalender memuat ~22 hari bursa
...
weights_dict = None  # Default (Balanced = dibagi rata 1/24)

if scoring_mode == "Fokus Return (Profit)":
    weights_dict = { 'Total_Return': 0.25, 'Return_1W': 0.25, 'Return_1M': 0.25, 'Return_3M': 0.25 }
...
with tab_overview:
    ...
    if not ranked_products_all.empty:
        with st.spinner("Mengkalkulasi jejak peringkat 7 hari terakhir..."):
            history_ranks = get_7d_ranking_history(df_all_instruments, benchmark_series_sliced, risk_free_rate, custom_weights=weights_dict)
...
with tab_performance:
    ...
    start_date_str = cutoff_date.strftime('%d %b %Y')
    ...
    - **Momentum Return:** Persentase profit 1W, 1M, 3M, dan Total Return.
    - **Risk & Reward:** Volatility, Sharpe Ratio, Alpha, dan Beta.
    - **Konsistensi Peringkat (12 Metrik):** Mengukur frekuensi dan *streak* produk masuk ke Top 5, 10, dan 20 pada irisan kalender Harian, Mingguan (7d), 2-Mingguan (14d), dan 3-Mingguan (21d).
    - **Total Skor:** Rata-rata persentil dari 20 metrik di atas (bobot 5% per kolom).""")

    perf_type = st.radio("Pilih Kategori Aset", ["Equity", "Bonds"], horizontal=True, key="perf_type_radio")
    ...
    if not df_perf.empty and ranked_perf is not None and not ranked_perf.empty:
        ...
        with col_top1:
            ...
            top5_return = full_performance.sort_values('Total_Return', ascending=False).head(5)
            df_show_return = top5_return[['Total_Return']].copy()
            df_show_return['Total_Return'] = (df_show_return['Total_Return'] * 100).round(2).astype(str) + '%'
            df_show_return = df_show_return.rename(columns={'Total_Return': f'Total Return'})
            df_show_return.index.name = 'Nama Produk'
            st.dataframe(df_show_return, use_container_width=True)
            
        with col_top2:
            st.markdown("**🌟 Top 5 Skor Komposit (Risk-Adjusted)**")
        ...
        # --- 2. ANALISIS MENDALAM PRODUK UNGGUL ---
        st.subheader(f"🔍 Mengapa Produk Ini Unggul?")
        st.caption(f"Analisis komparatif berdasarkan kinerja {date_option} terakhir dibandingkan rata-rata kategori.")
        
        # Menggunakan list dari Top 5 Skor Komposit sebagai opsi dropdown
        top_5_list = top5_score.index.tolist()
        if top_5_list:
            selected_top = st.selectbox(f"Pilih Produk {title} dari Top 5 Skor Komposit untuk dianalisis:", top_5_list, key="perf_select_top")
            if selected_top:
                st.write(f"**Analisis Mendalam: {selected_top}**")
                metrics_top = metrics_perf.loc[selected_top]
                reasons = []
                if pd.notna(metrics_top['Total_Return']):
                    avg_ret = metrics_perf['Total_Return'].mean() * 100
                    val_ret = metrics_top['Total_Return'] * 100
                    if val_ret > avg_ret:
                        reasons.append(f"✅ **Pertumbuhan Kuat (Return)**: Mencetak profit sebesar **{val_ret:.2f}%**, mengungguli rata-rata {title.lower()} lain **{avg_ret:.2f}%**.")
                ...
                if pd.notna(metrics_top['Volatility']):
                    avg_vol = metrics_perf['Volatility'].mean() * 100
                ...
                if pd.notna(metrics_top['Alpha']):
                    avg_alpha = metrics_perf['Alpha'].mean() * 100
                    val_alpha = metrics_top['Alpha'] * 100
                    if val_alpha > avg_alpha:
                        reasons.append(f"✅ **Alpha Positif**: **{val_alpha:.2f}%** > rata-rata **{avg_alpha:.2f}%**.")
                if not reasons:
                    reasons.append("ℹ️ Produk ini unggul karena kombinasi metrik yang secara umum stabil dan memadai.")
        ...
        st.subheader(f"📋 Tabel Metrik & Skor Lengkap - {title}")
        ...
        full_performance_display = full_performance.copy().sort_values('Total_Score', ascending=False)
        full_performance_display.index.name = 'Nama Produk'
        
        # 1. Klasifikasi Kelompok Metrik
        return_metrics = ['Total_Return', 'Return_1W', 'Return_1M', 'Return_3M']
        ...
        if scoring_mode == "Balanced (Semua Metrik)":
            active_metrics = return_metrics + risk_metrics + consist_metrics + climb_metrics
        ...
        # 3. Bangun Urutan Kolom (Hanya Nilai Mentah dan Total Skor)
        ...
        cols_to_format = {
            'Total_Return': "{:.2%}", 'Return_1W': "{:.2%}", 'Return_1M': "{:.2%}", 'Return_3M': "{:.2%}",
            'Volatility': "{:.2%}", 'Sharpe_Ratio': "{:.4f}", 'Beta': "{:.4f}", 'Alpha': "{:.2%}",
            'Total_Score': "{:.3f}"
        }
        ...
        st.subheader(f"🔥 Heatmap Analisis Lanjutan - {title}")
...
with tab_compare:
    ...
    st.info(f"""ℹ️ **Metodologi Metrik & Skor Komposit ({date_option} | {start_date_str} s/d {end_date_str}):**
                    - **Momentum Return:** Persentase profit 1W, 1M, 3M, dan Total Return.
                    - **Risk & Reward:** Volatility, Sharpe Ratio, Alpha, dan Beta.
                    - **Konsistensi Peringkat (12 Metrik):** Frekuensi & streak masuk Top 5/10/20.
                    - **Climbers/Laggards (4 Metrik):** Mengukur tren perpindahan peringkat harian saat ini dibandingkan posisi 1, 7, 14, dan 22 hari perdagangan sebelumnya (Positif = Naik Peringkat).
                    - **Total Skor:** Rata-rata persentil dari 24 metrik di atas (bobot ~4.16% per kolom).""")
    ...
    if len(selected_instruments) >= 1:
        ...
        fig_returns = px.line(returns_cum * 100, x=returns_cum.index, y=returns_cum.columns, title="Return Kumulatif (%)")
        fig_returns.add_hline(y=0, line_dash="dash", line_color="gray", opacity=0.5)
        fig_returns.update_layout(xaxis_title="Tanggal", yaxis_title="Return (%)", legend=legend_layout)
        ...
        for inst in selected_instruments:
            ...
            inst_nav = safe_slice(inst_nav_full, cutoff_date)
            roll_mean = safe_slice(roll_mean_full, cutoff_date)
            upper_1sd = safe_slice(upper_1sd_full, cutoff_date)
            lower_1sd = safe_slice(lower_1sd_full, cutoff_date)
            upper_2sd = safe_slice(upper_2sd_full, cutoff_date)
            lower_2sd = safe_slice(lower_2sd_full, cutoff_date)
            upper_3sd = safe_slice(upper_3sd_full, cutoff_date)
            lower_3sd = safe_slice(lower_3sd_full, cutoff_date)

            fig_band = go.Figure()
            
            # Garis Putih agar terlihat di Dark Mode Streamlit
            fig_band.add_trace(go.Scatter(x=inst_nav.index, y=inst_nav, mode='lines', name='NAV Aktual', line=dict(color='white', width=2.5)))
            fig_band.add_trace(go.Scatter(x=roll_mean.index, y=roll_mean, mode='lines', name=f'Mean ({dynamic_window}d)', line=dict(color='blue', width=1.5, dash='dot')))
            
            fig_band.add_trace(go.Scatter(x=upper_1sd.index, y=upper_1sd, mode='lines', name='+1 SD', line=dict(color='rgba(44, 160, 44, 0.6)', width=1, dash='dash')))
            fig_band.add_trace(go.Scatter(x=lower_1sd.index, y=lower_1sd, mode='lines', name='-1 SD', line=dict(color='rgba(44, 160, 44, 0.6)', width=1, dash='dash')))
            
            fig_band.add_trace(go.Scatter(x=upper_2sd.index, y=upper_2sd, mode='lines', name='+2 SD', line=dict(color='rgba(255, 127, 14, 0.6)', width=1, dash='dash')))
            fig_band.add_trace(go.Scatter(x=lower_2sd.index, y=lower_2sd, mode='lines', name='-2 SD', line=dict(color='rgba(255, 127, 14, 0.6)', width=1, dash='dash')))
            
            fig_band.add_trace(go.Scatter(x=upper_3sd.index, y=upper_3sd, mode='lines', name='+3 SD', line=dict(color='rgba(214, 39, 40, 0.6)', width=1, dash='dash')))
            fig_band.add_trace(go.Scatter(x=lower_3sd.index, y=lower_3sd, mode='lines', name='-3 SD', line=dict(color='rgba(214, 39, 40, 0.6)', width=1, dash='dash')))
            
            fig_band.update_layout(title=f"Distribusi Harga & Pita Volatilitas: {inst}", xaxis_title="Tanggal", yaxis_title="NAV / Harga", legend=legend_layout, hovermode="x unified")
        ...
        sliced_ts_dict = {k: safe_slice(v, cutoff_date) for k, v in dynamic_ts.items()}
        ...
        if ts_data:
            if 'Alpha' in ts_data and not ts_data['Alpha'].empty:
                fig_alpha = px.line(ts_data['Alpha'], title=f"Pergerakan Alpha ({dynamic_window} Hari)")
            ...
            if 'Beta' in ts_data and not ts_data['Beta'].empty:
                fig_beta = px.line(ts_data['Beta'], title=f"Pergerakan Beta ({dynamic_window} Hari)")
            ...
            if 'Sharpe_Ratio' in ts_data and not ts_data['Sharpe_Ratio'].empty:
                fig_sharpe = px.line(ts_data['Sharpe_Ratio'], title=f"Pergerakan Sharpe Ratio ({dynamic_window} Hari)")
            ...
            if 'Volatility' in ts_data and not ts_data['Volatility'].empty:
                fig_vol = px.line(ts_data['Volatility'], title=f"Pergerakan Risk (2 Std Dev, {dynamic_window} Hari)")
