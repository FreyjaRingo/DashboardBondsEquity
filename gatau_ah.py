...
def load_all_data(start_date, end_date, custom_equity=None, custom_bond=None):
    """
    ...
    def fetch_and_clean_data(ticker_list, field, params, name="Data"):
        ...
        # Harus menggunakan get_data di sini, BUKAN get_history
    ...
    # Ekstrak Data
    df_equity_raw = fetch_and_clean_data(tickers_equity, ['TR.FundNAV.date', 'TR.FundNAV'], params, "Equity")
    df_bond_raw = fetch_and_clean_data(tickers_bond, ['TR.FundNAV.date', 'TR.FundNAV'], params, "Bonds")
    ...
    for col in valid_metrics:
        if col in df_scaled.columns:
            ...
            if len(values) > 0:
                ...
                if weights[col] < 0:
                    df_scaled[col + '_scaled'] = 1 - df_scaled[col + '_scaled']
...
def calculate_daily_leaderboard(price_data, days=7):
    """
    Menghitung perubahan peringkat harian dalam seminggu terakhir berdasarkan Return saja.
    """
    if price_data.shape[0] < days + 1:
        return pd.DataFrame()

    price_today = price_data.iloc[-1]
    price_7d_ago = price_data.iloc[-1 - days]
    ...
    if price_data.shape[0] > days + 1:
        ...
        price_8d_ago = price_data.iloc[-2 - days]
...
def calculate_daily_leaderboard_by_type(price_data, days=7):
    """Menghitung leaderboard terpisah untuk equity dan bonds"""
    if price_data.shape[0] < days + 1:
        return pd.DataFrame()
...
def validate_ticker(ticker, product_type):
    ...
    try:
        if product_type == "Equity":
            df = rd.get_data(universe=[ticker], fields=['TR.FundNAV'])
        else:
            df = rd.get_data(universe=[ticker], fields=['TR.FundNAV'])
...
with st.sidebar:
    ...
    if st.button("Hubungkan ke Refinitiv Workspace"):
...
with tab_leaderboard_split:
    ...
    if not df_lb.empty:
        leaderboard = calculate_daily_leaderboard_by_type(df_lb, days=7)
...
with tab_compare:
    ...
    if len(selected_instruments) >= 2:
        ...
        if chart_type == "Harga (Normalisasi)":
            ...
            df_normalized = df_compare.div(df_compare.iloc[0]) * 100
